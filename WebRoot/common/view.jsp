...
<%
	...
	String pdfSplitPath = conf.separate(conf.getConfig("path.swf", "")) + pdfdoc + "_" + pages + ".pdf";
	...
	if(!validatedConfig){
		...
		if("swf".equals(format) || "jpg".equals(format) || "png".equals(format) || "pdf".equals(format)){
			...
			if("png".equals(format)||"jpg".equals(format)){
				...
				if("true".equals(conf.getConfig("splitmode", "")))	{
					splitpdf split = new splitpdf(request);
					messages = split.convert(pdfdoc, pages);
					if(messages.length() == 0 || "[OK]".equals(messages))
						response.setContentType("application/pdf");
						outs.write(conf.file_get_contents(pdfSplitPath));
				}else{
					response.setContentType("application/pdf");
